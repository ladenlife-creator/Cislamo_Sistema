...
class GradingSystemService extends BaseAcademicService
{
    ...
    public function getGradingSystems(array $filters = [])
    {
        ...
        return $query->with(['gradeScales.ranges', 'school'])
    ...
    public function createGradeScale(GradingSystem $gradingSystem, array $data): GradeScale
    {
        ...
        $this->createDefaultGradeRanges($gradeScale);

        return $gradeScale->load('ranges');
    }
    ...
    /**
     * Create default grade ranges for grade scale
     */
    private function createDefaultGradeRanges(GradeScale $gradeScale): void
    {
        ...
        // Disable model events temporarily to avoid recursion
        GradeScaleRange::unsetEventDispatcher();

        foreach ($gradeLevels as $index => $level) {
            GradeScaleRange::create([
                'grade_scale_id' => $gradeScale->id,
                'min_value' => $level['min_percent'] ?? $level['numeric'],
                'max_value' => $level['max_percent'] ?? $level['numeric'],
                'display_label' => $level['display'],
                'description' => $level['description'] ?? null,
                'color' => $level['color'] ?? null,
                'gpa_equivalent' => $level['gpa'] ?? null,
    ...
    private function hasGradingSystemDependencies(GradingSystem $gradingSystem): bool
    {
        ...
        foreach ($gradingSystem->gradeScales as $scale) {
            foreach ($scale->ranges as $range) {
                if (\App\Models\V1\Academic\GradeEntry::where('letter_grade', $range->display_label)->exists()) {
                    return true;
                }
            }
        }
